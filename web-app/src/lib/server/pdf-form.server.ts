/**
 * PDF Form Server Service
 * Server-side only service for PDF form field mapping and filling using pdf-lib
 * This module uses Node.js fs/promises and should NEVER be imported in client components
 */
...
export class PDFFormServerService {
  private static instance: PDFFormServerService;
  ...
  static getInstance(): PDFFormServerService {
    if (!PDFFormServerService.instance) {
      PDFFormServerService.instance = new PDFFormServerService();
    }
    return PDFFormServerService.instance;
  }
  ...
  /**
   * Simple method to fill a PDF using template code
   */
  async fillPDF(
    templateCode: string,
    pdfDoc: PDFDocument,
    ...
    try {
      ...
      // Try to fill existing form fields first
      const formFields = form.getFields();
      ...
      // If no form fields or fields weren't filled, use coordinate-based placement
      if (formFields.length === 0) {
        console.log('No form fields found, using coordinate-based placement');
        
        // Load field mappings from the JSON configuration
        const fieldMappings = await this.loadFieldMappings(templateCode);
        console.log(`Loaded ${fieldMappings.length} field mappings for ${templateCode}`);
        
        if (fieldMappings.length > 0) {
          // Use the coordinate-based placement with proper mappings
          for (const mapping of fieldMappings) {
            if (!mapping.page || mapping.x === undefined || mapping.y === undefined) {
              continue;
            }
            ...
            // Convert from top-down coordinates to bottom-up (PDF coordinate system)
            const pdfY = height - mapping.y;
            ...
            if (mapping.type === 'checkbox') {
              ...
              page.drawText(formattedValue, {
                ...
                color: rgb(0, 0, 0),
                maxWidth: mapping.width,
                lineHeight: (mapping.fontSize || 10) * 1.2
              });
            }
            
            console.log(`Placed field ${mapping.fieldName} at (${mapping.x}, ${pdfY}) with value: ${formattedValue}`);
          }
        } else {
          console.warn(`No field mappings found for template ${templateCode}`);
        }
      }
  ...
  private getTemplatePath(templateCode: string): string {
    ...
    const templatePaths: Record<string, string> = {
      ...
      'BUYER_COUNTER_OFFER': '03-escrow-contingency/buyer-counter-offer',
      'SELLER_COUNTER_OFFER': '03-escrow-contingency/seller-counter-offer',
      'REQUEST_FOR_REPAIR': '01-buyers-offer/request-for-repair',
      'BUYER_CONTINGENCY_REMOVAL': '02-contingency-removal/buyer-contingency-removal',
      'LEAD_BASED_PAINT': '01-buyers-offer/lead-based-paint'
    };
...
export const pdfFormServerService = PDFFormServerService.getInstance();